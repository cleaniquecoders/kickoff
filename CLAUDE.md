...
11. **Bootstrap artisan calls must not depend on Redis being up**: The stub `.env` ships with `CACHE_DRIVER=redis`, `SESSION_DRIVER=redis`, and `REDIS_PASSWORD=CHANGE_ME_BEFORE_DEPLOY`. During `runTasks()`, bootstrap commands run *before* the developer has had a chance to start Docker or fix the password — any artisan command that boots **Spatie Laravel Data**'s structure cache (e.g. `operations:install`, which uses `OptionsData::from(...)` internally) will resolve `CACHE_STORE` → `CACHE_DRIVER=redis` and crash with `RedisException: ERR AUTH <password> called without any password configured`. `runTasks()` wraps these calls in `withSafeBootstrapEnv()` which `putenv()`s `CACHE_STORE=array`, `CACHE_DRIVER=array`, `SESSION_DRIVER=array` for the duration. **Always wrap new bootstrap artisan calls in `withSafeBootstrapEnv()`** when they might boot the application container.
