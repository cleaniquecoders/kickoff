    ...
    private function createDeveloperMenuGroup(): MenuItem
    {
        return (new MenuItem)
            ...
            ->setVisible(fn () => (Route::has('security.audit-trail.index') && Gate::allows('view.audit-logs'))
                || (Route::has('mcp-kit.tasks') && Gate::allows('mcp-kit.view-tasks'))
                || (Route::has('ops.settings.mcp-tokens') && Gate::allows('ops.access.dashboard'))
                || (Route::has('mcp-tokens') && Gate::allows('access.mcp'))
            ...
            ->addChild($this->child('MCP', ['mcp-kit.tasks', 'ops.settings.mcp-tokens', 'mcp-tokens'], 'cpu', ['mcp-kit.view-tasks', 'ops.access.dashboard', 'access.mcp'], 'Model Context Protocol tasks'))
