<div>
    ...
    {{-- Audit Logs (click a row to view details) --}}
        ...
        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                @forelse ($audits as $audit)
                    <tr wire:key="audit-{{ $audit->uuid }}"
                        wire:click="openDetail('{{ $audit->uuid }}')"
    ...
    {{-- Read-only audit detail flyout (opened by row click) --}}
    <x-flyout name="audit-detail" size="lg" wire:model="showDetail">
        @php $audit = $this->selectedAudit; @endphp
        @if ($audit)
            <div class="mb-4 space-y-1">
                <flux:heading size="lg">{{ __('Audit Details') }}</flux:heading>
                <flux:subheading>{{ class_basename($audit->auditable_type) }} #{{ $audit->auditable_id }}</flux:subheading>
            </div>
